...
class GPIO(metaclass=MetaGPIO):
  """Base class for all GPIO objects. Implements methods shared by all the
  these objects, and ensures their dataclass is MetaGPIO."""
  ...
  def get_data(self) -> Optional[Union[list, Dict[str, Any]]]:
    ...
    print(f"WARNING ! The get_data method is not defined for the GPIO "
  ...
  def set_cmd(self, *cmd) -> None:
    ...
    print(f"WARNING ! The GPIO {type(self).__name__} has incoming links but"
  ...
  def start_stream(self) -> None:
    ...
    print(f"WARNING ! The GPIO {type(self).__name__} does not define the "
  ...
  def get_stream(self) -> Optional[List[np.ndarray]]:
    ...
    print(f"WARNING ! The GPIO {type(self).__name__} has downstream links but"
  ...
  def make_zero(self, delay: float) -> None:
    ...
    if not buf:
      print(f"No data acquired when zeroing the channels of the GPIO "
    ...
    for values in zip(*buf):
      try:
        ...
        self._compensations = list()
        print(f"WARNING ! Cannot calculate the offset for the GPIO "
